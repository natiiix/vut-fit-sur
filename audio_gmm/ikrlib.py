...
def gellipse(mu, cov, n=100, *args, **kwargs):
    ...
    if mu.shape != (2, 1) or cov.shape != (2, 2):
        raise RuntimeError(
            'mu must be a two element vector and cov must be 2 x 2 matrix')

    d, v = eigh(4 * cov)
    d = diag(d)
    t = np.linspace(0, 2 * pi, n)
    x = v.dot(np.sign(d)).dot(np.sqrt(np.abs(d))).dot(
        array([np.cos(t), np.sin(t)])) + mu
...
def logpdf_gauss(x, mu, cov):
    assert(mu.ndim == 1 and len(mu) == len(cov) and (
        cov.ndim == 1 or cov.shape[0] == cov.shape[1]))
...
def rand_gauss(n, mu, cov):
    if cov.ndim == 1:
        cov = np.diag(cov)
    assert(mu.ndim == 1 and len(mu) == len(cov)
           and cov.ndim == 2 and cov.shape[0] == cov.shape[1])
    d, v = eigh(cov)
...
def train_gmm(x, ws, mus, covs):
    ...
    of weights Ws.
    """
    gamma = np.vstack([np.log(w) + logpdf_gauss(x, m, c)
                       for w, m, c in zip(ws, mus, covs)])
    ...
    if covs[0].ndim == 1:  # diagonal covariance matrices
        ...
        covs = np.array([(gamma[i]*x.T).dot(x)/gammasum[i] - mus[i]
                         [:, newaxis].dot(mus[[i]]) for i in range(len(ws))])
...
def train_generative_linear_classifier(x, class_id):
    ...
    data_cov = (true_cov * len(true_data) +
                false_cov * len(false_data)) / len(x)
    inv_cov = np.linalg.inv(data_cov)
    w0 = -0.5 * true_mean.dot(inv_cov).dot(true_mean) + \
        0.5 * false_mean.dot(inv_cov).dot(false_mean)
...
def mel_filter_bank(nfft, nbands, fs, fstart=0, fend=None):
    ...
    cbin = np.round(mel_inv(np.linspace(mel(fstart), mel(
        fend), nbands + 2)) / fs * nfft).astype(int)
    mfb = np.zeros((nfft // 2 + 1, nbands))
    for ii in range(nbands):
        mfb[cbin[ii]:  cbin[ii+1]+1,
            ii] = np.linspace(0., 1., cbin[ii+1] - cbin[ii] + 1)
        mfb[cbin[ii+1]:cbin[ii+2]+1,
            ii] = np.linspace(1., 0., cbin[ii+2] - cbin[ii+1] + 1)
    return mfb
...
def raw8khz2mfcc(dir_name):
    ...
    """
    features = {}
    # for f in glob('/mnt/c/Users/Aiden/Documents/School/SUR/SUR_projekt2019-2020/IKR_demos_py/gID_data/male/train/*.raw'):
    for f in glob(dir_name + '/*.raw'):
        print('Processing file: ', f)
        features[f] = mfcc(np.fromfile(f, np.int16, -1, ''),
                           200, 120, 256, 8000, 23, 13)
    return features
...
def png2fea(dir_name):
    ...
    for f in glob(dir_name + '/*.png'):
        print('Processing file: ', f)
        features[f] = mmread(f)  # .astype(np.float64)
    return features
...
def demo_gmm():
    ...
    def hard_decision(x): return logpdf_gauss(x, mu1, cov1) + \
        log(p1) > logpdf_gauss(x, mu2, cov2) + log(p2)

    # compute posterior probability for class X1
    def x1_posterior(x): return logistic_sigmoid(logpdf_gauss(
        x, mu1, cov1) + log(p1) - logpdf_gauss(x, mu2, cov2) - log(p2))
    ...
    for i in range(30):
        im = plt.plot(x1[0, :], x1[1, :], 'r.') + \
            plt.plot(x2[0, :], x2[1, :], 'b.')
        for j in range(m1):
            im += gellipse(mus1[:, j], covs1[j, :, :],
                           100, 'r', lw=round(ws1[j] * 10))
        for j in range(m2):
            im += gellipse(mus2[:, j], covs2[j, :, :],
                           100, 'b', lw=round(ws2[j] * 10))
        ims.append(im)

        (ws1, mus1, covs1, ttl1) = train_gmm(x1, ws1, mus1, covs1)
        (ws2, mus2, covs2, ttl2) = train_gmm(x2, ws2, mus2, covs2)

        print('Total log-likelihood: %s for class X1; %s for class X2' %
              (ttl1, ttl2))
    ...
    def hard_decision(x): return logpdf_gmm(x, ws1, mus1, covs1) + \
        log(p1) > logpdf_gmm(x, ws2, mus2, covs2) + log(p2)
    plot2dfun(hard_decision, ax, 100, ax=ax1)
    ax1.plot(x1[0, :], x1[1, :], 'r.')
    ax1.plot(x2[0, :], x2[1, :], 'b.')
    for j in range(m1):
        gellipse(mus1[:, j], covs1[j, :, :], 100,
                 'r', lw=round(ws1[j] * 10), ax=ax1)
    for j in range(m2):
        gellipse(mus2[:, j], covs2[j, :, :], 100,
                 'b', lw=round(ws2[j] * 10), ax=ax1)

    def x1_posterior(x): return logistic_sigmoid(logpdf_gmm(
        x, ws1, mus1, covs1) + log(p1) - logpdf_gmm(x, ws2, mus2, covs2) - log(p2))
    plot2dfun(x1_posterior, ax, 100, ax=ax2)
    ax2.plot(x1[0, :], x1[1, :], 'r.')
    ax2.plot(x2[0, :], x2[1, :], 'b.')
    for j in range(m1):
        gellipse(mus1[:, j], covs1[j, :, :], 100,
                 'r', lw=round(ws1[j] * 10), ax=ax2)
    for j in range(m2):
        gellipse(mus2[:, j], covs2[j, :, :], 100,
                 'b', lw=round(ws2[j] * 10), ax=ax2)
    plt.show()
